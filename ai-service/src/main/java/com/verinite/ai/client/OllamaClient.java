    ...
    public class OllamaClient {
        ...
        private String callOllamaInternal(String prompt, Integer maxTokensOverride, boolean jsonMode) {
            ...
            try {
                ...
                int numCtx = configRepo.findByConfigKey("ollama.context.length")
                        .map(OllamaConfig::getConfigValue).map(Integer::parseInt).orElse(8192);
                request.put("options", Map.of(
                        "temperature", Double.parseDouble(temperature),
                ...
                log.warn("Invalid numeric config for temperature/max_tokens/context.length — using defaults");
            }
