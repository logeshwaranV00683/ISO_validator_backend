...
--    FK col        → {parent_singular}_id
--    Soft-delete   → deleted_at DATETIME NULL  (NULL = alive)
--    Audit cols    → created_at / updated_at / created_by VARCHAR(50) / updated_by VARCHAR(50)
--    Boolean flags → BOOLEAN (MySQL maps to TINYINT(1))
--    Cross-service → store id + name snapshot — no FK across logical service boundaries
-- ================================================================
...
CREATE TABLE user_sessions (
    ...
    revoke_reason   VARCHAR(100)    NULL                              COMMENT 'LOGOUT | PASSWORD_CHANGED | ADMIN_REVOKE | ROLE_CHANGE',
    is_active       BOOLEAN         NOT NULL DEFAULT TRUE             COMMENT 'FALSE once revoked or expired',
    created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  COMMENT='JWT session tracking — supports per-jti revocation. Queried by jti on every protected request.';

-- ================================================================
-- 3. system_config
--    Auth-service owns this. No created_at/created_by — rows are
--    seeded at deploy time, only updates happen at runtime.
--    config_type drives how config_value is parsed at runtime.
--    is_sensitive masks value in all API responses.
-- ================================================================
CREATE TABLE system_config (
    ...
    config_type     ENUM('STRING','INTEGER','BOOLEAN','JSON') NOT NULL DEFAULT 'STRING'
                                                              COMMENT 'How to parse config_value at runtime',
    description     VARCHAR(500)    NULL,
    is_sensitive    BOOLEAN         NOT NULL DEFAULT FALSE     COMMENT 'TRUE = mask value in all API responses',
    updated_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    updated_by      VARCHAR(50)     NULL,
...
CREATE TABLE switch_profiles (
    ...
    environment           ENUM('bPROD','UAT','DEV') NOT NULL DEFAULT 'DEV',
    host                  VARCHAR(255)    NOT NULL,
    port                  INT             NOT NULL                    COMMENT 'TCP port 1–65535',
    timezone              VARCHAR(100)    NOT NULL DEFAULT 'UTC'      COMMENT 'IANA tz — for DE7/DE12/DE13 parsing',
    ...
    is_default            BOOLEAN         NOT NULL DEFAULT FALSE      COMMENT 'Only one row may be TRUE — enforced at service level',
    last_used_at          DATETIME        NULL                        COMMENT 'Updated every time this profile is used for validation',
  ...
  COMMENT='Switch connection profiles. Owns host/port/timezone. Rules live in validation_rules. Formats in message_formats.';

-- ================================================================
-- 5. message_formats
--    Each format belongs to one profile (profile_id FK).
--    xml_content is the live/current packager XML.
--    Full version history is in message_format_versions.
--    status column controls whether format is usable independent
--    of soft-delete — matches blueprint active/inactive concept.
-- ================================================================
CREATE TABLE message_formats (
    ...
    total_fields    INT             NOT NULL DEFAULT 128              COMMENT 'Total DE fields defined in this format (typically 64 or 128)',
    status          ENUM('active','inactive') NOT NULL DEFAULT 'active'
                                                                      COMMENT 'active = usable by validation engine; inactive = disabled',
...
--    Immutable history — rows are never updated after insert.
--    is_current=TRUE marks the active version (only one per format).
-- ================================================================
CREATE TABLE message_format_versions (
    ...
    validated_ok    BOOLEAN         NOT NULL DEFAULT FALSE            COMMENT 'TRUE = jPOS successfully loaded this XML',
...
CREATE TABLE validation_rules (
    id              BIGINT          NOT NULL AUTO_INCREMENT,
    profile_id      BIGINT          NOT NULL                          COMMENT 'Ref to switch_profiles.id — no FK (cross-domain boundary)',
    ...
    PRIMARY KEY (id),
    UNIQUE KEY  uk_rules_profile_mti_de  (profile_id, mti, de_number) COMMENT 'One rule per DE per MTI per profile',
...
-- ================================================================
-- 9. field_definitions
--    Drives the message builder UI — what DEs are available,
--    in what order, with what placeholder.
--    profile_name snapshot avoids cross-table join on display.
--    active flag allows disabling a field without deleting it.
-- ================================================================
CREATE TABLE field_definitions (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
    profile_id          BIGINT          NOT NULL                      COMMENT 'Ref to switch_profiles.id — no FK (cross-domain)',
    profile_name        VARCHAR(100)    NOT NULL                      COMMENT 'Snapshot — denormalized for display without join',
    ...
    active              BOOLEAN         NOT NULL DEFAULT TRUE          COMMENT 'FALSE = field disabled, hidden from builder',
    ...
    KEY         idx_fielddef_active         (active),
...
CREATE TABLE ai_prompt_templates (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY  uk_apt_scope_profile  (scope, profile_id)             COMMENT 'One template per scope+profile combo',
...
--     Immutable — no update after insert.
-- ================================================================
...
-- ================================================================
-- 12. ai_run_logs
--     Every Ollama API call is logged here regardless of outcome.
-- ================================================================
...
-- ================================================================
-- 13. ollama_config
--     AI-service owns this. Isolated from auth system_config.
--     config_type drives runtime parsing of config_value.
--     is_sensitive masks value in API responses.
-- ================================================================
CREATE TABLE ollama_config (
    id              BIGINT          NOT NULL AUTO_INCREMENT,
    config_key      VARCHAR(100)    NOT NULL,
    config_value    LONGTEXT        NOT NULL                          COMMENT 'LONGTEXT — some values may be large (e.g. system prompts)',
...
-- ================================================================
-- 14. validation_runs
--     Written async via RabbitMQ by history-service consumer.
--     raw_message stores the original hex for rerun capability.
--     original_run_id allows tracing rerun chains by ID.
--     updated_at tracks async enrichment (e.g. soft-delete).
-- ================================================================
CREATE TABLE validation_runs (
    ...
    environment             VARCHAR(100) NULL             COMMENT 'Switch profile environment captured at time of run — PROD | UAT | DEV',
    ...
    raw_message             LONGTEXT        NOT NULL                  COMMENT 'Original hex message — required for rerun',
    mti                     VARCHAR(4)      NULL,
    ...
    status                  ENUM('PASSED','FAILED','WARNED','PROCESSING','PARSE_ERROR') NOT NULL
                                                                      COMMENT 'PASSED=no errors, FAILED=CRITICAL errors, WARNED=warnings only',
    total_fields_present    INT             NOT NULL DEFAULT 0,
    total_fields_parsed     INT             NOT NULL DEFAULT 0        COMMENT 'Fields successfully parsed by jPOS (may differ from present)',
    ...
    hex_message_hash        VARCHAR(64)     NULL                      COMMENT 'SHA-256 of raw_message — used for rerun dedup detection',
    ...
    original_run_id         BIGINT          NULL                      COMMENT 'Points to original run row if is_rerun=TRUE',
    original_run_reference  VARCHAR(30)     NULL                      COMMENT 'Human-readable ref of original run if is_rerun=TRUE',
    ...
    KEY         idx_runs_original_id    (original_run_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- ================================================================
-- 17. audit_logs
--     IMMUTABLE — never UPDATE or DELETE any row here.
--     Written async via RabbitMQ from all services.
--     description stores a human-readable summary of the action.
-- ================================================================
CREATE TABLE audit_logs (
    ...
    description       VARCHAR(500)    NULL                            COMMENT 'Human-readable summary e.g. "Rule DE7 created for UAT Switch 0200"',
    correlation_id    VARCHAR(36)     NULL,
    ip_address        VARCHAR(45)     NULL,
